...
def create(cmd, client, resource_group_name, cluster_type, cluster_name, name, url, bucket_name=None,
    ...
    return sdk_no_wait(no_wait, client.begin_create_or_update, resource_group_name, cluster_rp,
                        cluster_type, cluster_name, name, flux_configuration)

def update(cmd, client, resource_group_name, cluster_type, cluster_name, name, kind=consts.GIT, url=None,
                    ...
                    known_hosts=None, known_hosts_file=None, access_key=None, secret_key=None, insecure=None,
                    suspend=None, kustomization=None, no_wait=False, yes=False):

    # Get Resource Provider to call
    cluster_rp = get_cluster_rp(cluster_type)
    validate_cc_registration(cmd)

    config = show(cmd, client, resource_group_name, cluster_type, cluster_name, name)
    if not kind:
    ...
    # kind type
    changed_source_kind = kind.lower() != convert_to_cli_source_kind(config.source_kind).lower()
    ...
    if kustomization:
        # Convert the Internal List Representation of Kustomization to Dictionary
        kustomization = {k.name: k.to_KustomizationPatchDefinition() for k in kustomization}
    ...
    return sdk_no_wait(no_wait, client.begin_update, resource_group_name, cluster_rp,
                        cluster_type, cluster_name, name, flux_configuration)

def create_kustomization(cmd, client, resource_group_name, cluster_type, cluster_name, name,
    ...
    current_config = show(cmd, client, resource_group_name, cluster_type, cluster_name, name)
...
def update_kustomization(cmd, client, resource_group_name, cluster_type, cluster_name, name,
                        kustomization_name, dependencies=None, timeout=None, sync_interval=None,
...
def delete_kustomization(cmd, client, resource_group_name, cluster_type, cluster_name, name,
...
def list_kustomization(cmd, client, resource_group_name, cluster_type, cluster_name, name):
    # Get Resource Provider to call
    cluster_rp = get_cluster_rp(cluster_type)
    validate_cc_registration(cmd)
...
def show_kustomization(cmd, client, resource_group_name, cluster_type, cluster_name, name,
                                   kustomization_name):
...
def delete(cmd, client, resource_group_name, cluster_type,
    ...
    return sdk_no_wait(no_wait, client.begin_delete, resource_group_name, cluster_rp, cluster_type,
                        cluster_name, name, force_delete=force)

def _validate_source_control_config_not_installed(cmd, resource_group_name, cluster_rp, cluster_type, cluster_name):
    ...
    configs = source_control_client.list(resource_group_name, cluster_rp, cluster_type, cluster_name)
...
def _validate_extension_install(cmd, resource_group_name, cluster_rp, cluster_type, cluster_name, no_wait):
    ...
    if not flux_extension:
        ...
        if not is_dogfood_cluster(cmd):
            extension = __add_identity(extension,
                                            resource_group_name,
                                            cluster_rp,
                                            cluster_type,
                                            cluster_name)
        ...
        sdk_no_wait(no_wait, extension_client.begin_create, resource_group_name, cluster_rp, cluster_type,
...
def __add_identity(cmd, extension_instance, resource_group_name, cluster_rp, cluster_type, cluster_name):
...
class GitRepositoryGenerator(SourceKindGenerator):
    def __init__(self, **kwargs):
        ...
        super().__init__(consts.GIT, consts.GIT_REPO_REQUIRED_PARAMS, consts.GIT_REPO_VALID_PARAMS)
...
class BucketGenerator(SourceKindGenerator):
    def __init__(self, **kwargs):
        # Common Pre-Validation
        super().__init__(consts.BUCKET, consts.BUCKET_REQUIRED_PARAMS, consts.BUCKET_VALID_PARAMS)
        super().validate_params(**kwargs)
