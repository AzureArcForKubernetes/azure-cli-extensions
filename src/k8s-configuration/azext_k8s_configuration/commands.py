...
def load_command_table(self, _):
    ...
    with self.command_group('k8s-configuration flux', k8s_configuration_fluxconfig_sdk, client_factory=k8s_configuration_fluxconfig_client, is_experimental=True) as g:
        ...
        g.custom_command('source create', 'flux_config_create_source', supports_no_wait=True)
        g.custom_command('kustomization create', 'flux_config_create_kustomization', supports_no_wait=True)
        g.custom_command('source update', 'flux_config_update_source', supports_no_wait=True)
        g.custom_command('kustomization update', 'flux_config_update_kustomization', supports_no_wait=True)
        g.custom_command('source delete', 'flux_config_delete_source', confirmation=True, supports_no_wait=True)
        ...
        g.custom_command('resume', 'flux_config_resume', supports_no_wait=True)
