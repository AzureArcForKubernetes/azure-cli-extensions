...
class DataProtectionKubernetes(DefaultExtension):
    ...
    def Create(
        ...
        # Current scope of DataProtection Kubernetes Backup extension is 'cluster'
        if scope == 'namespace':
            raise InvalidArgumentValueError(f"Invalid scope '{scope}'. This extension can only be installed at 'cluster' scope. Please check https://aka.ms/tsg for more information.")

        scope_cluster = ScopeCluster(release_namespace=release_namespace)
        ext_scope = Scope(cluster=scope_cluster, namespace=None)
        
        if cluster_type.lower() != 'managedclusters':
            raise InvalidArgumentValueError(f"Invalid cluster type '{cluster_type}'. This extension can only be installed for managed clusters. Please check https://aka.ms/tsg for more information.")
    ...
    def __validate_and_map_config(self, configuration_settings, validate_bsl=True):
        ...
        input_configuration_settings = {key: value for key, value in configuration_settings.items()}
